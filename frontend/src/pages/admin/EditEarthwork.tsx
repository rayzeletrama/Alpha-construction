...
export const EditEarthwork = () => {
  const queryClient = useQueryClient();
  const [formData, setFormData] = useState<any>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [uploadingField, setUploadingField] = useState<string | null>(null);

  // État pour un nouvel article technique (Terrassement)
  const [newArticle, setNewArticle] = useState({
    badge: "Focus Terrassement",
    title: "",
    slug: "", // Liaison dynamique
    text: "",
    image: "https://images.unsplash.com/photo-1581094794329-c8112a89af12",
  });

  const { data: page, isLoading } = useQuery({
    queryKey: ["page", "terrassement"],
    queryFn: async () => (await api.get("/v1/pages/terrassement")).data,
  });
  ...
  const mutation = useMutation({
    mutationFn: (newContent: any) =>
      api.put("/v1/pages/terrassement", { content: newContent }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["page", "terrassement"] });
      toast.success("Contenu Terrassement sauvegardé !");
    },
    onError: () => toast.error("Erreur lors de la sauvegarde"),
  });

  // ✅ GESTION DE L'UPLOAD AVEC COMPRESSION CLIENT (Évite les 502 de Render)
  const handleImageUpload = async (
    ...
    try {
      ...
      const uploadData = new FormData(); // ✅ Changé pour éviter le conflit avec 'formData'
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-20 px-4 md:px-0">
      ...
      <section className="space-y-8">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="flex-1 space-y-4">
                  {/* CHAMP SLUG BLEU POUR LIAISON DYNAMIQUE */}
                  <div className="bg-blue-50/50 p-2 rounded-sm border border-blue-100">
                    ...
                    <input
                      ...
                      placeholder="ex: terrassement-masse"
                      value={art.slug || ""}
                      onChange={(e) => {
        ...
        >
          + Ajouter un nouveau bloc technique
        </button>
      </section>
      ...
      >
        <div className="space-y-5">
          <input
            className="w-full p-4 bg-gray-50 border border-gray-100 outline-none focus:border-primary font-bold"
            placeholder="Titre de la technique"
          ...
          <input
            ...
            placeholder="Slug (ex: terrassement-fondations)"
          ...
          <button
            onClick={() => {
              ...
              toast.success("Technique ajoutée au brouillon");
            }}
